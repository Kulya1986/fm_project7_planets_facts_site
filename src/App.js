...
import BottomSummary from "./components/BottomSummary/BottomSummary";
import PlanetInformation from "./components/PlanetInformation/PlanetInformation";
...
export default function App() {
  ...
  function handleActiveInfoTabChange(name) {
    setActiveInfoTab(name);
  }

  const activePlanetInfo = data.filter(
    (item) => item.name.toLowerCase() === activePlanet
  )[0];

  function handleSetActivePlanet(name) {
  ...
  return (
    <div id="container">
      <Navigation data={data} onTabClick={handleSetActivePlanet} />
      <main>
        <PlanetInformation
          planet={activePlanetInfo}
          activeInfoTab={activeInfoTab}
          activeInfoTabChange={handleActiveInfoTabChange}
        />
        <BottomSummary planet={activePlanetInfo} />
      </main>
    </div>
  );
}
